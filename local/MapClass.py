...
class SemanticMap():
    ...
    def match_predicted_object_with_gt_IoU(self,img_id,mask_id):
        predicted_mask = self.refined_prediction_masks[img_id][mask_id].copy()
        groundt_mask = self.semantic_observations[img_id].copy()

        overlaped_labels = groundt_mask[predicted_mask == 1]
        unique_labels = np.unique(overlaped_labels)
        ...
        for label in unique_labels:
            gt_object_mask = (groundt_mask == label)

            intersection = np.logical_and(predicted_mask, gt_object_mask).sum()
            union = np.logical_or(predicted_mask, gt_object_mask).sum()
