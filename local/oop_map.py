...
def draw_pose_graph(graph, title="Pose Graph", save_path=None):
    ...
    for obj in graph.extended_objects:
    	if obj.position is None:
    	    continue
    	x,y = -obj.position[0], obj.position[1]
    	plt.plot(x,y, "go")
    	plt.text(x+0.02, y+0.02, f"Obj{obj.parent_id}", fontsize=7, color="green")
...
def cluster_semantic_objects(subs, pose_graph, similarity_matrix, angle_threshold=15):
    for cluster_index, group in enumerate(subs):
        ...
        for node_id, mask_id in group:
            ...
            if node.theta is None or node.centroids.get(mask_id) is None:
            	continue
